...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagOn_Off)
    {
        info->event[info->num] = EVENT_On_Off;
        info->num++;
        dataPoints->valueOn_Off = gizStandardDecompressionValue(On_Off_BYTEOFFSET,On_Off_BITOFFSET,On_Off_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagCold_Warm)
    {
        info->event[info->num] = EVENT_Cold_Warm;
        info->num++;
        dataPoints->valueCold_Warm = gizStandardDecompressionValue(Cold_Warm_BYTEOFFSET,Cold_Warm_BITOFFSET,Cold_Warm_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueOn_Off != cur->valueOn_Off)
    {
        GIZWITS_LOG("valueOn_Off Changed\n");
        ret = 1;
    }
    if(last->valueCold_Warm != cur->valueCold_Warm)
    {
        GIZWITS_LOG("valueCold_Warm Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(On_Off_BYTEOFFSET,On_Off_BITOFFSET,On_Off_LEN,(uint8_t *)devStatusPtr,dataPoints->valueOn_Off);
    gizStandardCompressValue(Cold_Warm_BYTEOFFSET,Cold_Warm_BITOFFSET,Cold_Warm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCold_Warm);
